...
PAGERANK_PKL_PATH = "/home/einavra/PageRank.pkl"
PAGERANK_WEIGHT = 0.1
...
def load_pagerank():
    ...
    if _pagerank is not None:
        return _pagerank
    with open(PAGERANK_PKL_PATH, "rb") as f:
        _pagerank = pickle.load(f)   # dict: doc_id -> float
    return _pagerank
...
# Public API
# **********
def search_api(query, k=100, body_weight=0.6, title_weight=0.4, pagerank_weight=0.15):
    # normalize weights to sum to 1
    w_sum = body_weight + title_weight + pagerank_weight
    if w_sum <= 0:
        body_w, title_w, pr_w = 0.6, 0.4, 0.0
    else:
        body_w = body_weight / w_sum
        title_w = title_weight / w_sum
        pr_w = pagerank_weight / w_sum
    ...
    # PageRank boost - loaded once into memory
    if combined and pr_w > 0:
        pr = load_pagerank()
        pr_vals = [pr.get(int(doc_id), 0.0) for doc_id in combined.keys()]
        pr_max = max(pr_vals) if pr_vals else 0.0

        if pr_max > 0:
            for doc_id in list(combined.keys()):
                combined[doc_id] += pr_w * (pr.get(int(doc_id), 0.0) / pr_max)
